                    ...
                    match block.name:
                        ...
                        case "table":
                            if not "CD" in block.select_one("th").get_text():
                                continue
                            _tracks = []
                            for _row in list(more_itertools.chunked(block.select("tr"), n=2))[1:]:
                                row_1 = _row[0].select("td")
                                row_2 = _row[1].select("td")
                                _tracks.append(Track(
                                    order=int(row_1[0].get_text(strip=True)),
                            ...
                            match block.attrs.get("class", []):
                                ...
                                case ["data2"]:
                                    ...
                                    if "収録内容未定" in block.get_text(strip=True):
                                        ...
                                        for _row in list(more_itertools.chunked(block.select("tr")[1:], n=2)):
                                            row_1 = _row[0].select("td")
                                            row_2 = _row[1].select("td")
                                            _tracks.append(Track(
                                                order=int(row_1[0].get_text(strip=True)),
...
if __name__ == '__main__':
    _columns = [
        "release_title", "artist", "release_date", "label", "release_type",
        "disc_type", "catalog_number", "track_order", "track_title", "lyricist", "composer",
        "arranger", "singer"
    ]
    hp_list = []
    ...
    for row in hp_list:
        key = (row["catalog_number"], row["track_title"], row["track_order"])
    ...
    df.sort_values(
        by=["release_date", "catalog_number", "track_order"],
    ...
    # Save to SQLite
    db_path = "upfront_releases.db"
    conn = sqlite3.connect(db_path)
    try:
        # Convert date objects to strings for SQLite
        df["release_date"] = df["release_date"].astype(str)
        df.to_sql("releases", conn, if_exists="replace", index=False)
        print(f"Successfully saved {len(df)} tracks to {db_path} (table: releases)")
    finally:
        conn.close()
